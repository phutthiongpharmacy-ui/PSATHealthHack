...
export function CustomSelect<T extends string>({ value, options, placeholder, ariaLabel, onChange, error }: CustomSelectProps<T>) {
  ...
  return <div className="relative">
    <button
      ...
      className={`flex h-12 w-full items-center justify-between gap-3 rounded-xl border bg-hh-bg px-4 text-left text-base text-white outline-none transition hover:border-hh-cyan/60 focus:border-hh-cyan focus:ring-2 focus:ring-hh-cyan/30 ${error ? "border-red-400/60" : "border-hh-border"}`}
    >
