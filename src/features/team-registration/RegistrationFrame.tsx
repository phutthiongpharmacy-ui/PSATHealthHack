...
export function RegistrationFrame({ step, children, compact = false }: { step: number; children: ReactNode; compact?: boolean }) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-hh-bg px-3 py-4 text-hh-text sm:px-6 sm:py-6 lg:px-8">
      ...
      <div className={`relative z-10 mx-auto ${compact ? "max-w-2xl" : "max-w-6xl"}`}>
        <header className="flex flex-col items-start gap-3 border-b border-hh-border/40 pb-4 sm:flex-row sm:items-center sm:justify-between">
          <Link href="/" className="inline-flex items-center gap-2 font-mono text-sm uppercase tracking-wide text-hh-text-muted transition hover:text-white">
        ...
        <nav aria-label="ขั้นตอนการลงทะเบียน" className="my-6 sm:my-8">
            ...
            {steps.map((label, index) => {
              ...
              return (
                <li key={label} aria-current={active ? "step" : undefined} className="relative flex min-w-0 flex-1 flex-col items-center">
                  ...
                  <span
                    className={`relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 font-mono text-xs font-extrabold leading-none transition-all sm:h-10 sm:w-10 ${
                      completed
                        ? "border-hh-emerald bg-hh-emerald text-hh-bg"
                        : active
                        ? "border-hh-action bg-hh-action text-black shadow-[0_0_18px_rgba(255,106,0,.65)]"
                  ...
                  <span
                    className={`mt-2 hidden min-h-8 px-1 text-center font-mono text-xs leading-tight sm:block ${
                      completed ? "text-hh-emerald" : active ? "font-bold text-hh-action" : "text-hh-text-muted/70"
                    }`}
                  >
                    0{number} / {label}
                  </span>
          ...
          <div className="mt-3 text-center sm:hidden">
            <span className="font-mono text-sm font-extrabold text-hh-action">ขั้นที่ {currentStep.number} จาก {currentStep.total}</span>
            <span className="mx-2 text-hh-border">•</span>
